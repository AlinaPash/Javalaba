        ...
        while (!findLink.isEmpty()) {
            ...
            if (currentPair.depth < maxDepth) {
                ...
                try {
                    ...
                    my_socket.close();//закрытие сокета
                } catch (SocketTimeoutException e) {
                    my_socket.close();
                }
            }
            viewedLink.add(currentPair);//добавление в список текущую пару
        }
        showResult(viewedLink);//вызов метод показа результата из списка
    }
    public static void main(String[] args) {
        String[] arg = new String[]{"http://hmpg.net/","2"};//URL-адрес и глубина поиска
        try {
            Process(arg[0], Integer.parseInt(arg[1]));
        } catch (NumberFormatException | IOException e) {//если введёные параметры не правильно прописаны

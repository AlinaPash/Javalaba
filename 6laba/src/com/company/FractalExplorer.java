...
public class FractalExplorer {
    ...
    private void drawFractal() {// отрисовка фрактала в JImageDisplay
        enableGUI(false);// отключаем интерфейс на момент рисования
    ...
    public void enableGUI(boolean b) {// включение - отключение gui
    ...
    public class FractalWorker extends SwingWorker<Object, Object> {
        ...
        protected Object doInBackground() throws Exception {
            ...
            for (int i = 0; i < displaySize; i++) {
                int count = fractalGenerator.numIterations(FractalGenerator.getCoord(
                        range.x, range.x + range.width, displaySize, i),
                        FractalGenerator.getCoord(
                                range.y, range.y+range.width, displaySize, y_coord));
                if (count == -1)
                    rgb[i] = 0;
                else {
